<script lang="ts">
	...
	let { value, interactive = false, disabled = false, onchange }: Props = $props();
	...
	const solidend = $derived(hoverval !== null && hoverval < value ? hoverval : value);
	const dimstart = $derived(hoverval !== null ? Math.min(value, hoverval) : null);
	const dimend = $derived(hoverval !== null ? Math.max(value, hoverval) : null);
	const showdim = $derived(dimstart !== null && dimend !== null && dimend > dimstart);
	const atvalue = $derived(hoverval !== null && Math.abs(hoverval - value) < 0.2);
...
<div
	...
	onmousemove={interactive && !disabled ? (e) => { hoverval = fromptr(e); } : undefined}
	onmouseleave={interactive && !disabled ? () => { hoverval = null; } : undefined}
